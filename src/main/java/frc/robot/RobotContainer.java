...
//    controller.button(XboxController.Button.kY.value).onTrue(new SequentialCommandGroupWrapper(new GroundPickup(arm, extender, gripper), "-Button-Ground-Pickup"));
    ...
    //arm move on stick
    controller.axisGreaterThan(XboxController.Axis.kRightY.value, 0.2).whileTrue(new ManualMoveArm(arm, -Constants.MANUAL_ARM_SPEED));
    controller.axisLessThan(XboxController.Axis.kRightY.value, -0.2).whileTrue(new ManualMoveArm(arm, Constants.MANUAL_ARM_SPEED));
